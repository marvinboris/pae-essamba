...
@section('content')
    <form action="{{ route('admin.projects.update', $project->id) }}" method="post" class="pt-3">
        @csrf
        <div class="form-group">
            <label for="title_en" class="control-label">English title</label>
            <input type="text" name="title_en" id="title_en" class="form-control" value="{{ $project->title_en }}" required autofocus>
        </div>

        <div class="form-group">
            <label for="title_fr" class="control-label">French title</label>
            <input type="text" name="title_fr" id="title_fr" class="form-control" value="{{ $project->title_fr }}" required>
        </div>
        
        <div class="form-group">
            <label for="summernote_en" class="control-label">English content</label>
            <textarea name="content_en" id="summernote_en" rows="10">{!! $project->content_en !!}</textarea>
            {{-- <div id="summernote"></div> --}}
        </div>
        
        <div class="form-group">
            <label for="summernote_fr" class="control-label">French content</label>
            <textarea name="content_fr" id="summernote_fr" rows="10">{!! $project->content_fr !!}</textarea>
            {{-- <div id="summernote"></div> --}}
        </div>
